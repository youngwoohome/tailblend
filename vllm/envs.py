...
if TYPE_CHECKING:
    ...
    VLLM_ENABLE_TAPER_PLUS: bool = False
    # Supported values:
    # - taper: paper-reproduction controller without memory/lifetime signals.
    # - taper_plus: paper-reproduction controller plus memory/lifetime signals.
    # - tail_blend: stock eager admission plus BoN-aware preemption.
    # - eager_tail_blend: deprecated alias for tail_blend.
    # - bon_taper: BoN parent-level utility/slack controller.
    # - bon_taper_plus: bon_taper plus memory/lifetime signals.
    # - bon_taper_len_predictor: bon_taper plus cheap live-branch logprob hints.
    VLLM_TAPER_PLUS_POLICY: str = "taper_plus"
    VLLM_TAIL_BLEND_PILOT_K: int = 0
    VLLM_TAIL_BLEND_PREEMPTION: bool = False
    # Supported TailBlend preemption modes include:
    ...
    VLLM_EAGER_TAIL_BLEND_PREEMPTION_MODE: str = "full"
    VLLM_EAGER_TAIL_BLEND_RESERVATION_Q: float = 0.0
    VLLM_TAPER_PLUS_MEMORY_THRESHOLD_BLOCKS: int = -1
    VLLM_TAPER_PLUS_STEP_TARGET_MS: float = 50.0
    VLLM_TAPER_PLUS_LATENCY_PER_SEQ_MS: float = 2.0
    VLLM_TAPER_PLUS_LATENCY_PROFILE_PATH: str = ""
    VLLM_TAPER_PLUS_TTFT_SLO_MS: float = 2000.0
    VLLM_TAPER_PLUS_TPOT_SLO_MS: float = 50.0
    VLLM_TAPER_PLUS_RHO: float = 1.0
    VLLM_TAPER_PLUS_UTILITY: str = "linear"
    VLLM_TAPER_PLUS_ONLINE_CALIBRATION: bool = True
    VLLM_TAPER_PLUS_RECORD_LATENCY_SAMPLES: bool = False
    VLLM_TAPER_PLUS_CALIBRATION_ALPHA: float = 0.08
    VLLM_TAPER_PLUS_SAFETY_FACTOR: float = 1.10
    VLLM_TAPER_PLUS_EAGER_WHEN_UNPRESSURED: bool = False
    VLLM_TAPER_PLUS_BRANCH_OVERDUE_BOOST: bool = True
    VLLM_TAPER_PLUS_LOG_STEP_STATS: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # TAPER+: experimental proactive width regulation for parallel sampling.
    ...
    "VLLM_TAIL_BLEND_PILOT_K": lambda: int(
        os.getenv(
            ...
            os.getenv("VLLM_EAGER_TAIL_BLEND_PILOT_K", "0"),
        )
    ),
    "VLLM_TAIL_BLEND_PREEMPTION": lambda: bool(
        int(
            os.getenv(
                "VLLM_TAIL_BLEND_PREEMPTION",
                os.getenv("VLLM_EAGER_TAIL_BLEND_PREEMPTION", "0"),
            )
        )
    ),
    "VLLM_TAIL_BLEND_PREEMPTION_MODE": lambda: os.getenv(
        "VLLM_TAIL_BLEND_PREEMPTION_MODE",
        os.getenv("VLLM_EAGER_TAIL_BLEND_PREEMPTION_MODE", "full"),
    ),
    "VLLM_TAIL_BLEND_RESERVATION_Q": lambda: float(
        os.getenv(
    ...
    "VLLM_EAGER_TAIL_BLEND_PILOT_K": lambda: int(
        os.getenv(
            "VLLM_EAGER_TAIL_BLEND_PILOT_K",
            os.getenv("VLLM_TAIL_BLEND_PILOT_K", "0"),
        )
    ),
    "VLLM_EAGER_TAIL_BLEND_PREEMPTION": lambda: bool(
        int(
            os.getenv(
                "VLLM_EAGER_TAIL_BLEND_PREEMPTION",
                os.getenv("VLLM_TAIL_BLEND_PREEMPTION", "0"),
            )
        )
    ),
    "VLLM_EAGER_TAIL_BLEND_PREEMPTION_MODE": lambda: os.getenv(
        "VLLM_EAGER_TAIL_BLEND_PREEMPTION_MODE",
        os.getenv("VLLM_TAIL_BLEND_PREEMPTION_MODE", "full"),
    ),
    "VLLM_EAGER_TAIL_BLEND_RESERVATION_Q": lambda: float(
        os.getenv(
            "VLLM_EAGER_TAIL_BLEND_RESERVATION_Q",
            os.getenv("VLLM_TAIL_BLEND_RESERVATION_Q", "0.0"),
        )
    ),
    # If negative, the scheduler uses 5% of GPU KV blocks as the memory gate.
    "VLLM_TAPER_PLUS_MEMORY_THRESHOLD_BLOCKS": lambda: int(
        os.getenv("VLLM_TAPER_PLUS_MEMORY_THRESHOLD_BLOCKS", "-1")
    ),
    # TAPER+ latency target and warm-start predictor.
    "VLLM_TAPER_PLUS_STEP_TARGET_MS": lambda: float(
        os.getenv("VLLM_TAPER_PLUS_STEP_TARGET_MS", "50.0")
    ),
    "VLLM_TAPER_PLUS_LATENCY_PER_SEQ_MS": lambda: float(
        os.getenv("VLLM_TAPER_PLUS_LATENCY_PER_SEQ_MS", "2.0")
    ),
    "VLLM_TAPER_PLUS_LATENCY_PROFILE_PATH": lambda: os.getenv(
        "VLLM_TAPER_PLUS_LATENCY_PROFILE_PATH", ""
    ),
    "VLLM_TAPER_PLUS_TTFT_SLO_MS": lambda: float(
        os.getenv("VLLM_TAPER_PLUS_TTFT_SLO_MS", "2000.0")
    ),
    ...
    "VLLM_TAPER_PLUS_RECORD_LATENCY_SAMPLES": lambda: bool(
        int(os.getenv("VLLM_TAPER_PLUS_RECORD_LATENCY_SAMPLES", "0"))
    ),
    "VLLM_TAPER_PLUS_CALIBRATION_ALPHA": lambda: float(
        os.getenv("VLLM_TAPER_PLUS_CALIBRATION_ALPHA", "0.08")
    ),
    "VLLM_TAPER_PLUS_SAFETY_FACTOR": lambda: float(
        os.getenv("VLLM_TAPER_PLUS_SAFETY_FACTOR", "1.10")
    ),
    "VLLM_TAPER_PLUS_EAGER_WHEN_UNPRESSURED": lambda: bool(
        int(os.getenv("VLLM_TAPER_PLUS_EAGER_WHEN_UNPRESSURED", "0"))
    ),
    "VLLM_TAPER_PLUS_BRANCH_OVERDUE_BOOST": lambda: bool(
        int(os.getenv("VLLM_TAPER_PLUS_BRANCH_OVERDUE_BOOST", "1"))
    ),
    "VLLM_TAPER_PLUS_LOG_STEP_STATS": lambda: bool(
        int(os.getenv("VLLM_TAPER_PLUS_LOG_STEP_STATS", "0"))
    ),
