...
class KVCacheManager:
    ...
    def get_remaining_free_blocks(self) -> int:
        """Return currently free GPU KV-cache blocks for TAPER+ gating."""
        return self.block_pool.get_num_free_blocks()

    def estimate_future_footprint(self, request: Request) -> int:
        """Estimate additional GPU KV blocks needed until completion.

        TAPER+ uses this as a proactive memory-lifetime signal. Long-lived
        branches use the conservative max-token bound. Short-lived branches use
        a tighter predictor based on the branch's ``prob_short`` score.
        """
        remaining_output_tokens = max(
            0, request.max_tokens - request.num_output_tokens
        )
        if remaining_output_tokens == 0:
            return 0

        if request.is_short_lived():
        ...
        target_num_tokens = min(target_num_tokens, self.max_model_len)
        return self.coordinator.get_num_blocks_to_allocate(
            ...
            total_computed_tokens=min(
                request.num_computed_tokens, self.max_model_len
            ),
            num_tokens_main_model=target_num_tokens,
            apply_admission_cap=True,
        )

    def estimate_future_footprint_for_output_len(
        self,
        request: Request,
        target_output_tokens: int,
    ) -> int:
        """Estimate GPU KV blocks for a planning-only output length.

        This does not change the request's real generation limit. It lets a
        scheduler evaluate opportunistic work with a predicted length while
        preserving the actual ``max_tokens`` stopping condition.
        """
        target_output_tokens = max(
            request.num_output_tokens,
            min(request.max_tokens, target_output_tokens),
        )
        target_num_tokens = min(
            request.num_prompt_tokens + target_output_tokens,
            self.max_model_len,
        )
        return self.coordinator.get_num_blocks_to_allocate(
            request_id=request.request_id,
