...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if self.taper_plus_policy == "eager_tail_blend":
            self.taper_plus_policy = "tail_blend"
        if self.taper_plus_policy not in (
            "taper",
            "taper_plus",
            "tail_blend",
            "bon_taper",
            ...
            raise ValueError(
                "VLLM_TAPER_PLUS_POLICY must be 'taper', 'taper_plus', "
                "'tail_blend', 'bon_taper', 'bon_taper_plus', or "
                "'bon_taper_len_predictor', "
                f"got {self.taper_plus_policy!r}."
            )
        self.taper_plus_memory_threshold_blocks = (
            envs.VLLM_TAPER_PLUS_MEMORY_THRESHOLD_BLOCKS
        )
        self.tail_blend_pilot_k = max(0, envs.VLLM_TAIL_BLEND_PILOT_K)
        self.tail_blend_preemption = envs.VLLM_TAIL_BLEND_PREEMPTION
        self.tail_blend_preemption_mode = envs.VLLM_TAIL_BLEND_PREEMPTION_MODE
        if self.tail_blend_preemption_mode not in (
            "remaining",
            "remaining_recompute",
            "remaining_parent",
            "full",
        ...
        self.taper_plus_latency_width_ms = max(
            0.001, self.taper_plus_latency_per_seq_ms
        )
        self.taper_plus_latency_context_ms_per_1k = 0.0
        self.taper_plus_calibration_samples = 0
        self.taper_plus_latency_samples: list[dict[str, float]] = []
        self._load_taper_plus_latency_profile()
        self.bon_taper_controller = BoNTaperController(self)
        self.bon_taper_len_predictor = BoNTaperLenPredictor(self)
        self.tail_blend_controller = TailBlendController(self)
        self.eager_tail_blend_controller = self.tail_blend_controller
        self.num_cumulative_preemptions = 0
        self.num_cumulative_predictor_preemptions = 0
        self.num_cumulative_preempted_computed_tokens = 0
        self.num_cumulative_preempted_output_tokens = 0
    ...
    @staticmethod
    def _is_taper_plus_decode_candidate(request: Request) -> bool:
        return (
    ...
    def _get_taper_plus_memory_threshold(self) -> int:
        if self.taper_plus_memory_threshold_blocks >= 0:
            return self.taper_plus_memory_threshold_blocks
        return max(1, self.kv_cache_manager.block_pool.num_gpu_blocks // 20)

    def _predict_taper_plus_step_ms(
        self, decode_width: int, context_tokens: int = 0
    ) -> float:
        if decode_width <= 0:
            return 0.0
        context_k = context_tokens / 1000.0
        predicted_ms = (
            self.taper_plus_latency_base_ms
            + self.taper_plus_latency_width_ms * decode_width
            + self.taper_plus_latency_context_ms_per_1k * context_k
        )
        return max(0.0, predicted_ms) * self.taper_plus_safety_factor

    @staticmethod
    def _get_float_profile_value(
    ...
    def _load_taper_plus_latency_profile(self) -> None:
        """Warm-start the TAPER latency model from an offline profile JSON."""
        if not self.taper_plus_latency_profile_path:
            return

        profile_path = Path(self.taper_plus_latency_profile_path)
        with profile_path.open("r", encoding="utf-8") as f:
            profile = json.load(f)

        if "coefficients" in profile and isinstance(profile["coefficients"], dict):
        ...
        width_ms = self._get_float_profile_value(
            ...
            "latency_width_ms",
            "per_seq_ms",
            default=self.taper_plus_latency_width_ms,
        )
        context_ms_per_1k = self._get_float_profile_value(
            profile,
            "context_ms_per_1k",
            "latency_context_ms_per_1k",
            default=self.taper_plus_latency_context_ms_per_1k,
        )
        safety_factor = self._get_float_profile_value(
            profile, "safety_factor", default=None
        )

        assert base_ms is not None
        assert width_ms is not None
        assert context_ms_per_1k is not None
        self.taper_plus_latency_base_ms = max(0.0, base_ms)
        self.taper_plus_latency_width_ms = max(0.001, width_ms)
        self.taper_plus_latency_context_ms_per_1k = max(0.0, context_ms_per_1k)
        if safety_factor is not None:
            self.taper_plus_safety_factor = max(1.0, safety_factor)

        logger.info(
            "Loaded TAPER latency profile from %s: base=%.4fms, "
            "width=%.4fms, context=%.4fms/1k, safety=%.3f",
            profile_path,
            self.taper_plus_latency_base_ms,
            self.taper_plus_latency_width_ms,
            self.taper_plus_latency_context_ms_per_1k,
            self.taper_plus_safety_factor,
        )

    def _get_taper_plus_step_features(
        ...
        return decode_width, context_tokens, prefill_tokens

    def _update_taper_plus_latency_model(
        self,
        observed_ms: float,
        decode_width: int,
        context_tokens: int,
        ...
        if (
            not self.enable_taper_plus
            or not self.taper_plus_online_calibration
            or decode_width <= 0
        ):
            return

        # Keep the decode-step model focused. Prefill-heavy steps have a
        # different cost surface and would make branch-width regulation too
        # conservative after prompt bursts.
        if prefill_tokens > 0:
            return

        if observed_ms <= 0.0 or observed_ms > self.taper_plus_step_target_ms * 20:
            return

        alpha = self.taper_plus_calibration_alpha
        if alpha <= 0.0:
            return
        ...
        self.taper_plus_calibration_samples += 1

    def _record_taper_plus_latency_sample(
        self,
        observed_ms: float,
        predicted_ms: float,
        decode_width: int,
        ...
        if (
            not self.enable_taper_plus
            or not self.taper_plus_record_latency_samples
            or decode_width <= 0
            or prefill_tokens > 0
            or observed_ms <= 0.0
        ):
            return
        if (
            self.taper_plus_step_target_ms > 0.0
    ...
    def _get_taper_plus_group_deadline_s(
        ...
        sibling branch as an independent SLO owner. A parallel-stage request
        receives progress whenever any admitted branch advances, so its
        next-token TPOT clock is the most recent branch progress in the group.
        """
        if all(request.num_output_tokens == 0 for request in requests):
    ...
    def _get_taper_plus_branch_overdue_ms(
        ...
        """Return how long a sibling branch has waited past its own deadline.

        The logical parent deadline is useful for TTFT/TPOT SLO accounting, but
        it can hide sibling starvation: one active branch can keep refreshing the
        parent clock while other branches wait for many decode steps. This helper
        gives the planner a local pressure signal for those deferred branches.
        """
        return max(0.0, now_s - self._get_taper_plus_deadline_s(request)) * 1000.0

    def _taper_plus_marginal_utility(self, granted: int) -> float:
        """Operator utility curve used by the Algorithm 1 greedy planner."""
        if self.taper_plus_utility == "concave":
            return (granted + 1) ** 0.5 - granted**0.5
        return 1.0

    def _get_taper_plus_admitted_running_req_ids(
        ...
        if self.taper_plus_policy in (
            ...
            return self.bon_taper_controller.plan_admitted_req_ids(token_budget)
        use_taper_plus_memory_gate = self.taper_plus_policy == "taper_plus"
        ...
        all_decode_groups: set[str] = set()
        baseline_groups: set[str] = set()
        active_decode_width = 0
        active_context_tokens = 0

        for request in self.running:
            group_id = self._taper_plus_group_id(request)
            is_decode_candidate = self._is_taper_plus_decode_candidate(request)
            ...
            if group_id not in baseline_groups:
                # BuildBaseline(requests): protect exactly one branch per
                # active BoN parent before spending any opportunistic width.
                admitted_req_ids.add(request.request_id)
        ...
        memory_threshold = self._get_taper_plus_memory_threshold()
        virtual_free_blocks = self.kv_cache_manager.get_remaining_free_blocks()
        if (
            self.taper_plus_eager_when_unpressured
            and self.taper_plus_step_target_ms > 0.0
            and len(self.waiting) == 0
            and len(self.skipped_waiting) == 0
            and virtual_free_blocks >= memory_threshold * 2
        ):
            return {request.request_id for request in self.running}
        ...
        if self.taper_plus_step_target_ms <= 0.0:
            return admitted_req_ids
        ...
        group_slack_ms = {
            ...
            * 1000.0
            for group_id in all_decode_groups
        }
        min_slack_ms = min(group_slack_ms.values())
        baseline_step_ms = self._predict_taper_plus_step_ms(
            active_decode_width, active_context_tokens
        )
        budget_ms = baseline_step_ms + self.taper_plus_rho * max(
            0.0, min_slack_ms - baseline_step_ms
        )

        granted_by_group: dict[str, int] = defaultdict(int)
        infeasible_groups: set[str] = set()
        available_extra_slots = max(0, token_budget - active_decode_width)
        eps = 1e-6

        # Algorithm 1: repeatedly admit the feasible request with the largest
        # marginal utility per predicted marginal latency cost.
        while available_extra_slots > 0:
            ...
            for group_id, deferred in deferred_by_group.items():
                if group_id in infeasible_groups or not deferred:
                    continue
                ...
                # The global min-slack budget protects the most urgent request,
                # but it can over-throttle unrelated BoN groups. Also consider
                ...
                branch_overdue_ms = (
                    ...
                    if self.taper_plus_branch_overdue_boost
                    else 0.0
                )
                candidate_budget_ms = max(budget_ms, group_budget_ms)
                if branch_overdue_ms > 0.0:
                    # Logical parent progress can hide a sibling branch that has
                ...
                if use_taper_plus_memory_gate:
                    ...
                    if (
                        under_memory_pressure or over_future_budget
                ...
                dt = predicted_step_ms - baseline_step_ms
            ...
            granted_by_group[best_group_id] += 1
            deferred_by_group[best_group_id].pop(0)
            ...
            if use_taper_plus_memory_gate:
                virtual_free_blocks = max(
                    0,
                    virtual_free_blocks
                    - self.kv_cache_manager.estimate_future_footprint(best_request),
                )

        return admitted_req_ids

    def _get_taper_plus_admission_log_counts(
        self, admitted_req_ids: set[str] | None
    ) -> tuple[int, int, int, int, int]:
        """Summarize BoN/parallel decode admission for optional debug logs."""
        group_to_candidates: dict[str, list[Request]] = defaultdict(list)
        for request in self.running:
            if self._is_taper_plus_decode_candidate(request):
        ...
        if admitted_req_ids is None:
            ...
            admitted_regulated_branches = sum(
                1
                for requests in regulated_groups
                for request in requests
                if request.request_id in admitted_req_ids
            )
        deferred_regulated_branches = (
            total_regulated_branches - admitted_regulated_branches
        )
        max_group_width = max(
            (len(requests) for requests in regulated_groups), default=0
        )
        return (
            len(regulated_groups),
            total_regulated_branches,
            admitted_regulated_branches,
            deferred_regulated_branches,
            max_group_width,
        )
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if self._pause_state == PauseState.PAUSED_ALL:
            ...
            token_budget = 0
        local_prefix_cache_queries = 0
        local_prefix_cache_hits = 0
        connector_prefix_cache_query_tokens = 0
        connector_prefix_cache_hit_tokens = 0
        ...
        taper_plus_log_counts = (0, 0, 0, 0, 0)
        if self.taper_plus_log_step_stats:
            taper_plus_log_counts = self._get_taper_plus_admission_log_counts(
                taper_plus_admitted_running_req_ids
            )
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    ...
                    if used_predictor_preemption:
                        self.num_cumulative_predictor_preemptions += 1
                    if preempted_req_index < req_index:
                        req_index -= 1
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.taper_plus_log_step_stats:
                        local_prefix_cache_queries += request.num_tokens
                        local_prefix_cache_hits += num_new_local_computed_tokens
                    ...
                    if self.connector is not None:
                        ...
                        if self.taper_plus_log_step_stats:
                            connector_prefix_cache_query_tokens += (
                                connector_prefix_cache_queries
                            )
                            connector_prefix_cache_hit_tokens += (
        ...
        taper_plus_decode_width = 0
        taper_plus_context_tokens = 0
        ...
        if self.enable_taper_plus:
            ...
            ) = self._get_taper_plus_step_features(num_scheduled_tokens)
            taper_plus_predicted_step_ms = self._predict_taper_plus_step_ms(
                taper_plus_decode_width, taper_plus_context_tokens
            )
            taper_plus_step_start_s = time.perf_counter()

        if self.taper_plus_log_step_stats:
            (
                taper_plus_regulated_groups,
                taper_plus_regulated_branches,
                taper_plus_admitted_branches,
                taper_plus_deferred_branches,
                taper_plus_max_group_width,
            ) = taper_plus_log_counts
            logger.info(
                "TAPER step stats: policy=%s scheduled_tokens=%d "
                "prefill_tokens=%d decode_width=%d context_tokens=%d "
                "predicted_step_ms=%.3f local_prefix_hits=%d/%d "
                "external_prefix_hits=%d/%d regulated_groups=%d "
                "regulated_branches=%d admitted_branches=%d "
                "deferred_branches=%d max_group_width=%d running=%d "
                "waiting=%d skipped_waiting=%d free_blocks=%d",
                self.taper_plus_policy,
                total_num_scheduled_tokens,
                taper_plus_prefill_tokens,
                taper_plus_decode_width,
                taper_plus_context_tokens,
                taper_plus_predicted_step_ms,
                local_prefix_cache_hits,
                local_prefix_cache_queries,
                connector_prefix_cache_hit_tokens,
                connector_prefix_cache_query_tokens,
                taper_plus_regulated_groups,
                taper_plus_regulated_branches,
        ...
        scheduler_output = SchedulerOutput(
            ...
            taper_plus_step_start_s=taper_plus_step_start_s,
            taper_plus_decode_width=taper_plus_decode_width,
            taper_plus_context_tokens=taper_plus_context_tokens,
            taper_plus_prefill_tokens=taper_plus_prefill_tokens,
            taper_plus_predicted_step_ms=taper_plus_predicted_step_ms,
        )
    ...
    def _preempt_request(self, request: Request, timestamp: float) -> None:
        ...
        self.num_cumulative_preempted_computed_tokens += request.num_computed_tokens
        self.num_cumulative_preempted_output_tokens += request.num_output_tokens
    ...
    def update_from_output(
        ...
        if scheduler_output.taper_plus_step_start_s > 0.0:
            observed_step_ms = (
                time.perf_counter() - scheduler_output.taper_plus_step_start_s
            ) * 1000.0
            self._record_taper_plus_latency_sample(
                observed_ms=observed_step_ms,
                predicted_ms=scheduler_output.taper_plus_predicted_step_ms,
                decode_width=scheduler_output.taper_plus_decode_width,
                context_tokens=scheduler_output.taper_plus_context_tokens,
                prefill_tokens=scheduler_output.taper_plus_prefill_tokens,
            )
            self._update_taper_plus_latency_model(
                observed_ms=observed_step_ms,
                decode_width=scheduler_output.taper_plus_decode_width,
                context_tokens=scheduler_output.taper_plus_context_tokens,
                prefill_tokens=scheduler_output.taper_plus_prefill_tokens,
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Refresh cheap BoN lifetime hints. This path must not request or
            # expose logprobs; it only consumes scheduler-visible lengths.
            if (
                self.enable_taper_plus
                and self.taper_plus_policy == "bon_taper_len_predictor"
                and new_token_ids
            ):
                self.bon_taper_len_predictor.observe_request_output(
                    request, new_token_ids
                )
    ...
    def add_request(self, request: Request) -> None:
        ...
        if existing is not None:
            ...
            self.bon_taper_len_predictor.prepare_request(request)
            self._enqueue_waiting_request(request)
    ...
    def _free_request(
        ...
        if self.enable_taper_plus and self.taper_plus_policy == "tail_blend":
            self.tail_blend_controller.observe_finished_request(request)
        if (
            self.enable_taper_plus
            and self.taper_plus_policy == "bon_taper_len_predictor"
        ):
            self.bon_taper_len_predictor.observe_finished_request(request)
