...
class Request:
    def __init__(
        ...
        # TAPER+: Probability that this request/branch has a short remaining
        # lifetime. The scheduler can use this to prefer branches that are
        # likely to release KV cache soon.
        self.prob_short = 0.0
        self.taper_last_token_time = self.arrival_time
    ...
    def append_output_token_ids(
        ...
        self.taper_last_token_time = time.time()
        self.update_block_hashes()
    ...
    def update_prob_short(
        ...
        *,
        ...
        """Update the TAPER+ short-lifetime predictor.

        Prefer an EOS log-probability when a caller has it. Otherwise fall
        back to a length-progress heuristic so the signal is available without
        forcing every request to compute extra logprobs.
        """
        if self.sampling_params is not None and last_token_id is not None:
            if last_token_id in self.sampling_params.all_stop_token_ids:
                self.prob_short = 1.0
                return

        prob_from_eos = None
        if eos_logprob is not None:
            # eos_logprob is log(p_eos); exp and clamp into a probability.
            prob_from_eos = max(0.0, min(1.0, math.exp(eos_logprob)))

        max_tokens = max(1, self.max_tokens)
        length_progress = min(1.0, self.num_output_tokens / max_tokens)
        next_prob = max(length_progress, prob_from_eos or 0.0)
        self.prob_short = max(self.prob_short, next_prob)
